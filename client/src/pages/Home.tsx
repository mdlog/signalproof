...
export default function Home() {
  // `/?mode=measure` opens the console on that mode — the bottom tab bar and route pages link this way.
  const search = useSearch();
  const [, navigate] = useLocation();
  const initialMode = (() => { const m = new URLSearchParams(search).get("mode"); return m === "measure" || m === "proofs" || m === "api" ? (m as Mode) : "overview"; })();
  const [mode, setMode] = useState<Mode>(initialMode);
  useEffect(() => { setMode(initialMode); }, [initialMode]);
  const [selectedZone, setSelectedZone] = useState("Kota Tua");
  /** Map controls: a geohash prefix (prefixes are spatial, so "qxhf" is a neighbourhood) and a quality band. */
  const [mapQuery, setMapQuery] = useState("");
  const [mapBand, setMapBand] = useState<"all" | "strong" | "watch" | "attention">("all");
  ...
  /**
   * Every timestamped measurement on chain, oldest first, capped at the last TREND_WINDOW so the
   * x-axis stays readable. The badge says how many are plotted out of how many exist, so the number
   * can never be mistaken for the total shown elsewhere.
   */
  const TREND_WINDOW = 48;
  const trend = useMemo(() => {
    if (!isLive || !snap || snap.measurements.length === 0) return { points: [], total: 0 };
    const stamped = [...snap.measurements]
      .filter((m) => m.timestamp)
      .sort((a, b) => (a.timestamp ?? 0) - (b.timestamp ?? 0));
    const multiDay = stamped.length > 1 && (stamped[stamped.length - 1].timestamp ?? 0) - (stamped[0].timestamp ?? 0) > 86_400;
    const points = stamped.slice(-TREND_WINDOW).map((m) => ({
      time: new Date((m.timestamp ?? 0) * 1000).toLocaleString([], multiDay ? { month: "short", day: "2-digit", hour: "2-digit", minute: "2-digit" } : { hour: "2-digit", minute: "2-digit" }),
      quality: qualityScore(m.latencyMs, m.downloadMbps),
      latency: m.latencyMs ?? 0,
      area: m.areaHash,
      status: m.status,
    }));
    return { points, total: stamped.length };
  }, [isLive, snap]);
  const trendData = trend.points;

  /** What the map shows: the zones that match the search prefix and the quality band. */
  const mapZones = useMemo(() => {
    const prefix = mapQuery.trim().toLowerCase();
    return zones.filter((z) => (!prefix || z.code.startsWith(prefix)) && (mapBand === "all" || z.status === mapBand));
  }, [zones, mapQuery, mapBand]);
   ...
   */
  /** The dial spins while a measurement is in flight: sampling, at the gateway, or awaiting attestation. */
  const measuring = testState === "sampling" || testState === "submitted" || testState === "attesting";
