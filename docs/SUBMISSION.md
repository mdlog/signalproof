...
- A gateway that recomputes the commitment, recovers the contributor's EIP-191 signature, enforces freshness and uniqueness, and never returns signature, nonce or sessionHash.
- SourceBatchRegistry on Sepolia (relayer-gated), SignalProofSettlement and SignalProofBatchSettlement on CC3 Testnet — all three verified on their explorers, seven live cross-chain settlements including a 3-in-1 batch under one continuity proof, and a claim that moved 0.001 CTC from the pool to a contributor.
- A dashboard whose entire read model is a join of MeasurementSubmitted (Sepolia) and MeasurementVerified (Creditcoin). No database is needed to demo; a clone with RPC URLs shows real settlements. The coverage map decodes each geohash and draws the cell, never a point; areas that are not geohashes are listed as unmapped rather than placed.
...
Two things we found and fixed during the build, kept as inverted exploit tests: the first registry was permissionless, so anyone could name themselves payee, obtain a genuine proof and be paid for work nobody did (now relayer-gated, redeployed); and adding the batch route let one measurement be paid on both routes (now cross-checked against the sibling contract, skipped rather than reverted so one paid entry cannot strand a batch).

What is not built, stated plainly: no native app (the browser client is the measurement client), no anti-Sybil scoring, no buyer API (the "Create area brief" button is disabled and says so), read direction only, and end-to-end latency of 9–13 minutes dominated by attestation. Nothing in the UI claims an on-chain result that does not exist.

67 Foundry tests, 93 Vitest tests, pnpm smoke passes from a clean machine. MIT.
```
...
3. Source contract — SourceBatchRegistry (Sepolia, 0x15F3d74846a40bD67f8ce345B73ae4c400f759Dc)

Emits MeasurementSubmitted(bytes32 indexed measurementRoot, bytes32 indexed areaHash, address indexed contributor, bytes32 sessionHash, uint256 timestamp, uint256 latencyMs, uint256 downloadMbps). It stores no coordinates, no raw payload, no identity beyond the reward address. submitMeasurement reverts unless msg.sender == relayer. The first deployment was permissionless, and a pre-deploy review showed that emitter binding on Creditcoin cannot distinguish "genuine but unauthorised" from "genuine": an attacker could call the real registry naming themselves payee and settle a real proof. Authorisation therefore lives at admission; the relayer carries the gateway's signature check on-chain. contracts/test/AdvUnlimitedMint.t.sol keeps the original exploit with its assertions inverted.
...
5. **Deployed, verified, and settled on-chain** — three addresses, the 8.5-minute run with hashes, 3-in-1 batch, 67 + 93 tests.
6. **Three ways to get paid for nothing, found and closed before submission** — permissionless registry, double settlement, poisoned batch; each kept as an inverted exploit test.
...
| Deployed on a testnet | ✓ done | Sepolia `0x15F3…59Dc`; CC3 Testnet `0x8F14…b584`, `0x3B90…8C85` — all verified on explorers |
