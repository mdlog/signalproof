...
- [ ] Two minutes later, click **"Run another test"** is not available while attesting — so open a **second tab** on the same URL, connect the same wallet, and submit pre-warm #2. Two pre-warms give a window: whichever settles first between 1:30 and 3:00 of the take is the wow beat.
...
- [ ] Explorer pages pre-opened in tabs (they load slowly on camera): the registry on Etherscan, the settlement contract and the batch contract on Blockscout, and the three canonical transactions listed in `docs/VIDEO_SCRIPT.md`.
...
| 0:15–0:40 | Sidebar **"Proof queue"**. Click the top **"Settled"** card (it is an `<a>` to Blockscout). | Card text **"Proof verified on Creditcoin. Reward 0.001 CTC released."**, `tx …` and `block …` footer, **"Open on explorer"**. New tab: the `execute` call on `0x8F14…b584`, **Logs** tab shows `MeasurementVerified` with `queryId` | REAL | UNVERIFIED-IN-BROWSER |
...
| 1:10–1:30 | Stay on the panel until the **Evidence pipeline** label changes from **"sent to gateway"** to **"N blocks to attestation · ~M min"** (first `attestationProgress` poll, ≤ 20 s after the relayer tick). Then sidebar **"Proof queue"**, click the new **"Awaiting attestation"** card. | The **attested height / your block** box; card text **"Source block is mined; waiting for Creditcoin attestation."** New tab (Etherscan): `submitMeasurement` from the relayer to `0x15F3…59Dc`, `MeasurementSubmitted` with `contributor` = the connected wallet | REAL | UNVERIFIED-IN-BROWSER |
| 1:30–2:00 | **WOW.** The pre-warm card flips to **"Settled"** on the 15 s poll (badge on the "Proof queue" nav item, teal check icon). Click it → Blockscout. Then the **"Contributor reward"** card on the same page: accrued balance, **"Claim 0.001 CTC"** → confirm in wallet (on CC3) → **"Claim sent — view on Blockscout"**. | A phone measurement, proven by the Attestcoin precompile, paid on Creditcoin, claimed by the contributor's own wallet — in one screen | **REAL — must be.** If no pre-warm has settled by 2:00, use fallback F1 below | UNVERIFIED-IN-BROWSER |
...
| 2:30–2:50 | Sidebar **"Coverage overview"** → the map; then **"Data products"**. | Map subtitle **"aggregated from on-chain submissions"**; cells drawn from decoded geohashes; **"Weakest zones"** + **"Operator cue"**. Data products: area cards, and the honest footer **"Area cards are live; the buyer API is not built"** | REAL (map) / NOT BUILT and labelled as such (buyer API) | UNVERIFIED-IN-BROWSER |
...
```bash
git clone --recursive <GITHUB_URL>   # forge-std is a submodule; or `forge install` in contracts/
pnpm install
