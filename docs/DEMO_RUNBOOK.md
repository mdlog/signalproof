...
and the contract accrues the reward. Nothing on the demo path is mocked. The paid-access flow (Data products → Buy API
access) is also live and needs no pre-warm: pay → sign → key in about 40 seconds.
...
- [ ] **Pre-warm the chain read:** the first snapshot after a server start takes ~20 s (the public CC3 RPC answers `eth_getLogs` in 5–8 s per scan); until then the UI says "Prototype mode". Open the dashboard once and wait for "CC3 TESTNET · LIVE" before recording. After that every page paints instantly (stale-while-revalidate).
- [ ] `BUYER_ACCESS_SECRET` is set in `.env` (any long random string; the local `.env` has one). Without it the "Buy API access" card does not render and `/v1/areas/<cell>` is free — the paid-access beat needs it.
- [ ] The **buyer wallet** for the on-camera purchase holds ≥ 0.1 CTC on CC3 Testnet (0.05 price + gas). Use a second account, not the contributor's, so the leaderboard and the purchase read as two parties.
- [ ] `pnpm e2e:surface` passes (28 checks against the live chains; add `E2E_ACCESS_TX=<a previous payment hash>` to avoid paying again).
...
| 2:30–3:05 | Sidebar **"Coverage overview"** → the map (three cells); then **"Data products"** → **"Buy API access — 0.05 CTC"** → wallet: confirm the transfer → wallet: sign the message → the key card reads **"Key active until …"** → **"Area brief"** → the brief dialog. Then **"Verify a proof"** → paste the settled card's hash → Enter. | Map with 3 mapped cells; the purchase flow ("Confirm the payment in your wallet…" → "Sign to redeem…" → key); the brief's provenance table; the verifier's four green steps and the proof panel | REAL — the payment is a CC3 transaction into the settlement contract; the key is an HMAC over it | VERIFIED-IN-BROWSER 2026-09-13 (scripted wallet) |
| 3:05–3:15 | End card. | Repo URL, the one-liner, what is next | — | UNVERIFIED-IN-BROWSER |
