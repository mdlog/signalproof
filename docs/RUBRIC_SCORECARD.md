...
| **C5. Security posture and auditability** | 10% — CertiK audit credits for every winning team | 5 · 4 · 5 → **4.7** | 75 Foundry tests incl. a 6-test end-to-end suite and a signature vector shared with the client; four findings kept as **inverted exploits** (`AdvUnlimitedMint.t.sol`, `DoubleSettlement.t.sol`, `BatchPoison.t.sol`, `SourceBatchRegistry.t.sol` — the relayer can no longer forge attribution because the registry recovers the contributor's signature on-chain); a per-cell rate limit as stated policy; `BatchPoison.t.sol` characterises the batch route's accepted gaps; pull-payment rewards; secrets never reach the client; public API never returns `signature`/`nonce`/`sessionHash`. The README bullet that said payload signatures were "not yet cryptographically verified" contradicted `server/routers.ts` (which recovers the EIP-191 signer, 21 tests); corrected on 2026-09-11 in README, `docs/TECHNICAL_ARCHITECTURE.md`, `todo.md` and the deck | **20 minutes:** a short "Accepted risks" table in README pointing at the three `BatchPoison` characterisation tests |
...
**Weighted projected total (2026-09-11, after the hardening pass — signed registry, buyer API v1, proof inspector, self-settle, rate limit, container):** 0.35·4.7 + 0.15·4.7 + 0.20·3.3 + 0.10·3.7 + 0.10·4.7 + 0.10·4.0 = **4.25 / 5 = 85% — PASS** (bar for a top-3 finish in an 87-entry field under ESTIMATED weights: ≥ 80%).

**With the demo video in place** (C6 → 5.0): **4.35 / 5 = 87%.** Outside contributors on the map before recording (C3 → 3.7, C4 → 4.0) would add another two points.

**Lowest criterion:** C3, real-world use case and path to a product (3.3). Persona B — the one who
decides CEIP — now sees a queryable product surface (`/v1/areas`, the area brief) but still one
team's measurements in one city. The cheapest lift left is people, not code: 2–3 outside
contributors on the map from different cells before the video is recorded, and the live URL in
the Description.
...
2. **C3 Real-world path — 3.3.** The buyer API v1 and the area brief exist now; what remains is people: 2–3 outside contributors measuring from different cells before recording, so the map shows a network rather than one team.
