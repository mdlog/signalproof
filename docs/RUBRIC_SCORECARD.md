...
| **C5. Security posture and auditability** | 10% — CertiK audit credits for every winning team | 4 · 4 · 4 → **4.0** | 67 Foundry tests incl. a 6-test end-to-end suite; two findings kept as **inverted exploits** (`AdvUnlimitedMint.t.sol`, `DoubleSettlement.t.sol`); `BatchPoison.t.sol` characterises the batch route's accepted gaps; pull-payment rewards; secrets never reach the client; public API never returns `signature`/`nonce`/`sessionHash`. Persona C will notice that README "Known limitations" says payload signatures are "not yet cryptographically verified" while `server/routers.ts` recovers the EIP-191 signer and 21 tests cover it — **a doc/code contradiction reads as sloppiness** | **5 minutes:** correct the stale bullet in `README.md` (line ~430), `docs/TECHNICAL_ARCHITECTURE.md` §9 and `todo.md` to "signatures are verified at the gateway (signer recovery); they are not carried on-chain, so admission still trusts the relayer key". **20 minutes:** a short "Accepted risks" table in README pointing at the three `BatchPoison` characterisation tests |
| **C6. Submission completeness** | 10% — "Requirements for a complete submission"; a gate as much as a score | 2 · 2 · 2 → **2.0 today** | Form fields 6 (GitHub URL), 7 (deck PDF URL) and 8 (demo video URL) are blank; the repo is committed locally but not pushed; no deck; no video. Integration code, technical documentation (`docs/TECHNICAL_ARCHITECTURE.md`) and testnet deployment are done | Push the repo (10 min, verify `git clone --recursive` + `pnpm verify` from a clean directory), export the deck to PDF (2 h using the outline in `docs/SUBMISSION.md`), record the 3-minute video from `docs/VIDEO_SCRIPT.md` (2 h incl. pre-warm) → 5/5 |

**Weighted projected total (today):** 0.35·4.7 + 0.15·4.3 + 0.20·2.7 + 0.10·3.7 + 0.10·4.0 + 0.10·2.0 = **3.80 / 5 = 76% — AT RISK** (bar for a top-3 finish in an 87-entry field under ESTIMATED weights: ≥ 80%).

**Weighted projected total after the cheapest moves land** (C6 → 5.0, C3 → 3.3, C5 → 4.3): **4.25 / 5 = 85% — PASS.**
...
1. **C6 Submission completeness — 2.0.** Three required URLs are blank. Push the repo, publish the PDF, upload the video. Mechanical, ~4 h, and a DQ if skipped.
